...
public class RobotPlayer {
	...
	public static void seekOpponentHQ(RobotController rc)
			throws GameActionException {
		Direction toOpponent = rc.getLocation().directionTo(
				rc.senseEnemyHQLocation());
		int index = 0;
		Direction[] directions = Direction.values();
		while (!directions[index].equals(toOpponent))
			index++;
		for (int i = index; i < index + 8; i++) {
			int ii = i % 8;
			if (rc.canMove(directions[ii])
					&& rc.senseTerrainTile(rc.getLocation().add(directions[ii])) == TerrainTile.ROAD) {
				rc.sneak(directions[ii]);
				break;
			}
		}
	}
